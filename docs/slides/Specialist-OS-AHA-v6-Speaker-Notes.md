...
附录，2 分 19 秒，不计入前 15 分钟讲解。

点击视频播放。先看检测框怎样传给 SAM，再检查轮廓和深度图。
语音段先呈现转写，然后试听原音和降噪结果。音乐段处理完整歌曲，
依次播放同一位置的原曲、人声与伴奏。模型已经提前准备，推理关闭结果缓存，
视频以原速呈现。现场可在第 17 页后播放，也可直接打开独立 MP4。

音乐：Karissa Hobbs，Let's Go Fishin'，CC-BY-3.0。
录制与产物来源：docs/demo-video.md；output/demo/recordings/2026-09-07T11-02-58-372Z/manifest.json。
