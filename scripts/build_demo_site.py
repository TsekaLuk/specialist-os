...
</style><header class="top"><img src="assets/logo.png" alt="Specialist OS"><h1>Capability Workspace</h1><a href="index.html">返回能力画廊</a></header><main class="layout"><nav class="rail" id="rail"></nav><section class="center"><div class="eyebrow" id="family"></div><h2 id="title"></h2><p class="desc" id="desc"></p><div class="drop"><div><strong>输入资产</strong><p id="input">选择本地文件后运行真实 Specialist CLI</p><input id="file" type="file"></div></div><div class="result" id="result"></div></section><aside class="side"><h3>运行设置</h3><div class="field"><label>后端</label><select id="backend"><option>real</option></select></div><div class="field"><label>参数 JSON</label><textarea id="options" rows="6">{"no_cache":true}</textarea></div><button class="run" id="run">运行能力</button><div class="statusline"><span>状态</span><b id="status">已加载证据</b></div><div class="field evidence"><label>执行证据</label><pre id="evidence"></pre></div></aside></main><script type="module">
...
document.querySelector('#run').onclick=()=>{document.querySelector('#status').textContent='请用同名 capability 通过 Specialist CLI 或 Agent Skill 运行；本页只展示真实执行记录';};select(location.hash.slice(1)||cards[0].capability);
</script></html>'''
